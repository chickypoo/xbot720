...
const float KP = 0.61;                         // Proportion Control
const float KI = 2.73;                         // Integral Control
const float KD = 0.08;                         // Derivative Control
...
ISR(TIMER2_COMPA_vect){
  ...
  if(si == 250){
    code = testSequence4[indexSequence++];
    if(indexSequence == 12){
      runSEQ = false;
    }
    si = 0;
